...
class CustomOverView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<DateTimeCubit,DateTimeState>( 
      ...
      if (state is GetDateAndTimeFaluire){
        showToast(state.errorMassage);
      }
      },
    
    builder: (context,state){
      return state is GeyDateAndTimeLoadint? const CustomShimmerCategory(): overViewList(dateData: context.read<DateTimeCubit>().dateTimeData,);
    },
    );
  }
}